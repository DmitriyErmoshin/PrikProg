...
namespace RadioOwner
{
    public partial class Form1 : Form
    {
        ...
        {
            string resultQuery = Class1.POST("http://localhost/team/users.php", "select=1&role_id=2");
            int count = 0;
            ...
            int index = 0;
            for (int i = 0; i < resultArray.Length; i = i + 7)
            {
                if (Convert.ToInt32(Program.FindInfo(resultArray[i + 4])) == 2)
                {
                    ...
                    Users[index].Balance = Convert.ToInt32(Program.FindInfo(resultArray[i + 5]));
                    index++;
                }
            }
        }
